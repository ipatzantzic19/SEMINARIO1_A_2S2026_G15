...
import { profilePhotoSchema } from '../validation/profilePhoto'
...
/** Selector de foto reutilizable para registro y edición del perfil. */
function ProfilePhotoPicker({
  ...
  const preview = (value && previewUrl) || initialPreviewUrl

  return (
    <div className={isCompact ? 'w-full' : 'mt-2 w-full'}>
      <label
        className={`${isCompact ? 'sr-only' : 'mb-1.5 block'} font-body text-xs font-bold leading-[1.3] text-ink`}
        htmlFor={`${id}-file`}
      >
        {label}
      </label>
      {isCompact ? (
        <div
          className={`flex items-center gap-6 ${visibleError ? 'rounded-xl ring-2 ring-red-700 ring-offset-2 ring-offset-surface' : ''}`}
          aria-describedby={describedBy}
        >
          {preview && !hasPreviewError ? (
            <img
              className="h-31 w-31 shrink-0 rounded-full bg-mist object-cover"
              src={preview}
              alt="Vista previa de la foto de perfil"
              onError={() => setHasPreviewError(true)}
            />
          ) : (
            <div className="grid h-31 w-31 shrink-0 place-items-center rounded-full bg-mist font-display text-5xl font-bold text-ink">
              {fallbackText}
            </div>
          )}
