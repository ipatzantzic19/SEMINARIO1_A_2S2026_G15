# Contrato de API real — extraído del backend Node.js

Este documento describe **lo que el backend Node.js realmente hace hoy** (código en `api-node/src`), no lo que el diseño (`pra-1/CONTRATO_API.md`, `pra-1/openapi.yaml`) dice que debería hacer. Se generó leyendo controladores, servicios, DTOs, el guard JWT, el filtro de excepciones, `schema.sql` y `configuration.ts`. Al final hay una lista de discrepancias entre el diseño documentado, el código y el enunciado oficial (`CloudCinema_Practica_1.md`), para resolver con el compañero antes de escribir el equivalente en Python.

> Nota: el diseño en `pra-1/CONTRATO_API.md` sigue siendo la referencia "de jure". Este archivo es la referencia "de facto" (lo implementado). Cuando ambas difieren, lo digo explícitamente.
...
- Bucket real por defecto: `practica1-images-g15` (`configuration.ts`, override por `BUCKET_IMAGENES`). El enunciado pide `Practica1-Images-G#`; S3 exige minúsculas, así que el nombre en minúsculas ya está documentado como decisión en el manual técnico — no es un problema, pero confírmenlo con el nombre real del bucket creado en AWS.
...
1. **Catálogo de errores no implementado.** El filtro de excepciones (`http-exception.filter.ts`) solo mapea por código HTTP, no por caso de negocio, así que ningún `codigo` específico del catálogo documentado (`CREDENCIALES_INVALIDAS`, `CORREO_YA_REGISTRADO`, `USUARIO_NO_ENCONTRADO`, `PELICULA_NO_ENCONTRADA`, `PELICULA_NO_DISPONIBLE`, `PELICULA_YA_EN_LISTA`, `PELICULA_NO_ESTA_EN_LISTA`, `NO_AUTORIZADO`, `CONTRASENA_ACTUAL_INVALIDA`, `CONTRASENAS_NO_COINCIDEN`, `ID_PELICULA_INVALIDO`, `SIN_CAMBIOS_PROPUESTOS`, `IMAGEN_INVALIDA`) sale realmente en las respuestas de Node hoy. **Si construyo Python siguiendo el `CONTRATO_API.md` al pie de la letra, los dos servidores devolverán códigos de error distintos para el mismo caso**, lo cual contradice el requisito explícito del enunciado de que ambos backends expongan los mismos endpoints de forma transparente para el balanceador.
